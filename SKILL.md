...
- **串行调用**：受 M3 接口使用方式限制，按图片逐张串行识别；不要自行加并发。

耗时预期：
- 经验值按 **6-8 秒/张** 估算。
- 例如 78 张图，纯接口识别通常约 **8-10 分钟**；若中途超时后续传、叠加重试，**总耗时到 30 分钟仍属正常**。
- 执行者应预期这是长任务，不应因等待时间较长就误判为卡死。
...
- **置信度分流**：只有 `confidence = "high"` 的条目允许进入重命名与复制阶段；`medium` / `low` 必须视为未匹配，保留原文件名且不复制到 `Attachments/`。
- **推荐写法**：对 `medium` / `low` 条目，同时把 `refName` 置为 `""` 或 `"无匹配"`，让匹配表语义与脚本行为保持一致。
...
3. 自动将高置信度且已命名的配图复制到 **MD 文档同级** 的 `Attachments` 文件夹中。

这里有一个必须写清楚的边界：
- 本技能的目标目录始终是 `<MD所在目录>\Attachments`。
- 本技能**不会读取** Obsidian vault 的 `.obsidian/app.json` 中 `attachmentFolderPath` 配置，也不会尝试把文件复制到 vault 根或其他全局附件目录。
- `attachmentFolderPath` 只影响你在 Obsidian 里新增附件时默认存放到哪里；它不是本技能的输出路径配置源。
...
- **Obsidian 配置隔离**：不要根据 vault 的 `attachmentFolderPath` 推翻本技能的 `MD 同级/Attachments/` 设计
