---
description: 将下载目录中的 AI 配图做结构化识别和语义匹配，自动重命名并复制到 MD 同级 Attachments。当前主路径为 ServiceHub MiniMax-M3。
...
- `scripts/recognize-images-servicehub.ps1` 是主选识别路径
- `scripts/recognize-images.ps1` 仅在 ServiceHub 不可用时作为备用
